...
.source_fingerprint = function(project) {
  configured = unlist(Filter(Negate(is.null), list(project$paths$outputs, project$paths$releases)), use.names = FALSE)
  configured = if (length(configured)) basename(configured) else character()
...
.record_build_state = function(project, source = NULL) {
  ...
  output = .output_fingerprint(project, source = source)
  ...
  for (format in formats) {
    format_state[[format]] = source_fingerprint
  }
