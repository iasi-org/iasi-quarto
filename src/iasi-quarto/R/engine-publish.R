...
.publish_deploy_project = function(project,
                                    ...
                                    books,
                                    source = NULL) {
  publish_path = project$paths$publish
...
.publish_multiproject = function(plan, source = NULL) {
  publish_path = plan$projects[[1L]]$paths$publish
...
.normalise_regular_html = function(path, project) {
   ...
   for (file in files) {
      ...
      document = .normalise_html_global_resources(document, file, html_path)
...
.normalise_html_global_resources = function(document, file, html_path) {
   images = xml2::xml_find_all(
      document,
      "//img[contains(concat(' ', normalize-space(@class), ' '), ' iasi-ants-image ') and @src]"
   )

   if (!length(images)) return(document)

   for (image in images) {
      src = xml2::xml_attr(image, "src")
      if (is.na(src) || !nzchar(src)) next
      if (grepl("^(?:[A-Za-z][A-Za-z0-9+.-]*:|//|#)", src)) next

      resource = sub("[?#].*$", "", src)
      resource = sub("^/+", "", resource)
      target = file.path(html_path, utils::URLdecode(resource))

      if (!file.exists(target)) {
         basename_pattern = paste0("^", gsub("([][{}()+*^$|\\?.])", "\\\\\\1", basename(resource)), "$")
         ...
         if (length(candidates)) {
            normalised_resource = gsub("\\\\", "/", resource)
            candidate_keys = gsub("\\\\", "/", substring(candidates, nchar(html_path) + 2L))
            exact = which(tolower(candidate_keys) == tolower(normalised_resource))
            target = if (length(exact)) candidates[[exact[[1L]]]] else candidates[[1L]]
         }
      }

      if (!file.exists(target)) next

      href = .relative_href(dirname(file), target)
      suffix = sub("^[^?#]*", "", src)
      xml2::xml_attr(image, "src") = paste0(href, suffix)
   }
