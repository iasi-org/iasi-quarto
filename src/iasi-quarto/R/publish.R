...
#'   searched from `path` upwards and used as the publication source root.
#' @param releases Optional release directory name or path. Relative values are
#'   searched from `path` upwards. When `NULL`, publication uses the project's
#'   local `publish` directory.
#' @param iasi When `TRUE`, applies the IASI default for `releases` when no
...
#'
#' @export
publish = function(book = NULL, source = NULL, path = ".", outputs = NULL, releases = NULL, iasi = FALSE, force = FALSE) {
