...
# Fingerprint only author/source material. Generated build trees and local
# operational state are excluded so they cannot make their own source stale.
...
# Record source and output fingerprints per format after a successful build.
...
# Record the exact build tree and materialised publish tree. Destination paths
# are intentionally not persisted: state is local, and freshness is evaluated
# against the destination resolved for the current workspace invocation.
.record_publish_state = function(project,
                                  source = NULL,
                                  destination) {
  ...
  build = .publish_source_fingerprint(
    project,
    source = source
  )
...
# Publish is fresh only when both the build tree and the currently resolved
# destination match the fingerprints from the previous successful publication.
.publish_required = function(project,
                              source = NULL,
                              destination) {
  current_build = .publish_source_fingerprint(
    project,
    source = source
  )

  if (is.null(current_build)) {
    return(TRUE)
  }

  current_publish = .tree_fingerprint(
    destination
  )
